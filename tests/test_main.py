...
from telegram import Update, Chat, User, Message, InlineKeyboardMarkup
...
def test_create_new_user_menu():
    ...
    assert any("Request" in text for text in buttons_text), \
        f"Expected 'Request' button for new user, got: {buttons_text}"
    assert any("About" in text for text in buttons_text), \
...
def test_create_tenant_menu():
    ...
    for expected_text in expected:
        assert any(expected_text in text for text in buttons_text), \
...
def test_create_administrator_menu():
    ...
    expected = ['Submit Reading', 'Export', 'View Own Readings', 'Requests', 
                'Assign', 'Add', 'Manage']
    for expected_text in expected:
        assert any(expected_text in text for text in buttons_text), \
...
def test_highest_privilege_role_menu_wins():
    ...
    assert any('Assign' in text for text in buttons_text), \
