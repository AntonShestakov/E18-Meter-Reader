...
class Meter(Model):
    ...
    id = fields.IntField(pk=True)
    apartment = fields.ForeignKeyField('models.Apartment', related_name='meters', on_delete='CASCADE')
...
class UserRole(Model):
    ...
    id = fields.IntField(pk=True)
    user = fields.ForeignKeyField('models.User', related_name='roles', on_delete='CASCADE')
    role = fields.CharField(
        max_length=32,
        choices=['administrator', 'grayhound', 'tenant']
    )
...
class Reading(Model):
    ...
    id = fields.IntField(pk=True)
    meter = fields.ForeignKeyField('models.Meter', related_name='readings', on_delete='CASCADE')
    value = fields.DecimalField(max_digits=10, decimal_places=2)
    read_at = fields.DatetimeField(auto_now_add=True)
    submitted_by = fields.ForeignKeyField('models.User', related_name='readings', on_delete='RESTRICT')
    source = fields.CharField(
        max_length=32,
        choices=['numeric', 'photo']
    )
